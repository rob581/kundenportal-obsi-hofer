...
- **Nachträglich ergänzt (2026-09-22, Nutzerwunsch, im Rahmen von PROJ-7):** Kachel "Zu prüfen" ist jetzt ebenfalls als `Link` umgesetzt, analog zu den Status-Kacheln — zeigt zu `/uebersicht?zuPruefen=1`. Die 360-Tage-Grenzwertlogik (`ZU_PRUEFEN_TAGE`, `getZuPruefenCutoff`) wurde dafür aus `dashboard/queries.ts` in den neuen, geteilten Helper `src/lib/geraete/zu-pruefen.ts` verschoben (von `dashboard/queries.ts` weiterhin importiert) und zusätzlich von `getGeraeteList` (PROJ-3) für den neuen Filter genutzt — dieselbe Definition von "zu prüfen" an beiden Stellen, damit ein Klick auf die Kachel garantiert genau die Geräte zeigt, die in die angezeigte Zahl eingeflossen sind. Details zum neuen Filter selbst siehe PROJ-3 Implementation Notes.
- **BUG-3 gefunden und behoben (2026-09-22, Live-Fund auf Mobile):** Der gemeinsame `AppHeader` (Logo, Firma-Name, Navigation, "Firma wechseln", "Abmelden", Theme-Toggle) war als starre `grid-cols-3`-Zeile mit fester Höhe (`h-14`) gebaut — auf schmalen Viewports (375px) hatte er dafür schlicht zu viel Inhalt, wodurch sich alle Elemente überlagerten. Kein automatisierter Test hatte das aufgedeckt, da die Playwright-Suite nur Routen-Schutz prüft, nicht das visuelle Layout. **Fix:** Header ist jetzt mobil ein zweispaltiges Flex-Layout (Logo + Firma-Name links, ein neuer Hamburger-Button rechts via `sm:hidden`), ab dem `sm`-Breakpoint weiterhin das bisherige 3-Spalten-Grid. Der Hamburger öffnet ein neues `Sheet` (shadcn-Komponente, `npx shadcn add sheet`) mit Navigation, "Firma wechseln", "Abmelden" und Theme-Toggle untereinander. Neue Client-Component `src/components/app-header-mobile-menu.tsx` — nimmt die beiden Server Actions (`changeFirma`, `signOutEverywhere`) als Props entgegen (unterstütztes Next.js-Muster, Server Actions bleiben als Referenz seriali­sierbar). `AppHeader` selbst bleibt eine `async` Server Component, keine Änderung an der Datenabfrage.
- **Nachträglich geändert (2026-09-22, Nutzerwunsch):** Nav-Link "Übersicht" in "Geräte" umbenannt (Route bleibt `/uebersicht`, nur das sichtbare Label ändert sich) und die Reihenfolge der Header-Elemente auf Dashboard, Geräte, Prüfberichte, Firma wechseln, Abmelden, Sicherheit, Darkmode festgelegt — "Sicherheit" ist damit kein reiner Navigations-Link mehr in derselben Gruppe wie Dashboard/Geräte/Prüfberichte, sondern ein eigenständiger Link nach den beiden Formular-Buttons, kurz vor dem Theme-Toggle. Gleiche Reihenfolge in `app-header.tsx` (Desktop) und `app-header-mobile-menu.tsx` (mobiles Sheet-Menü) umgesetzt.
