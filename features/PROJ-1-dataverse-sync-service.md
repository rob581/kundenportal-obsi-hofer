...
| **Temporär (2026-09-21):** neuer Env-Schalter `CRON_NOTIFY_ON_SUCCESS` — bei sauberem Sync-Lauf jetzt ebenfalls eine Zusammenfassungs-Mail (bisher nur bei Warnungen/Fehlern) | Vercel Hobby zeigt Logs nur für die letzten 30 Minuten, der Cron läuft aber nachts um 3 Uhr — ohne diese Mail liesse sich ein erfolgreicher Lauf am nächsten Morgen nicht mehr nachvollziehen. Standardverhalten (kein Env-Var gesetzt) bleibt unverändert; einfach wieder entfernen, sobald die Überwachungsphase vorbei ist | 2026-09-21 |
...
**Zweites Auftreten des transienten `TypeError: fetch failed`-Fehlers (2026-09-17):** Beim ersten manuellen Sync-Lauf nach der `bemerkungen`-Ergänzung schlug ausschliesslich der `pruefberichte`-Batch-Upsert fehl (alle anderen 6 Entitäten liefen sauber durch); der sofortige Retry lief fehlerfrei durch — gleiches Muster wie beim ersten Auftreten (siehe oben, 2026-09-16). Kein Zusammenhang mit der Schema-Änderung erkennbar (kein Fehler, der auf ein Spalten-/Mapping-Problem hindeuten würde). Da das jetzt zum zweiten Mal auftritt, sollte eine automatische Retry-Logik in `run-sync.ts` bei einer künftigen Überarbeitung von PROJ-1 (z.B. via `/refine PROJ-1`) in Betracht gezogen werden, statt sich weiterhin nur auf den nächsten täglichen Lauf zu verlassen.
